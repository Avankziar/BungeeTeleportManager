...
import me.avankziar.btm.spigot.cmd.tree.ArgumentConstructor;
...
public class BTMCmdExecutor implements CommandExecutor
{
	...
	public BTMCmdExecutor(BTM plugin, CommandConstructor cc)
	{
		this.plugin = plugin;
	...
	public boolean onCommand(CommandSender sender, Command cmd, String lable, String[] args) 
	{
		...
		{
			BTM.logger.info("/%cmd% is only for Player!".replace("%cmd%", cc.getName()));
			return false;
		}
				...
				if(args[i].equalsIgnoreCase(ac.getName()))
				{
					if(length >= ac.minArgsConstructor && length <= ac.maxArgsConstructor)
					{
						if(player.hasPermission(ac.getPermission()))
						{
							...
							{
								BTM.logger.info("ArgumentModule from ArgumentConstructor %ac% not found! ERROR!"
		...
		///Deine Eingabe ist fehlerhaft, klicke hier auf den Text um &cweitere Infos zu bekommen!
		player.spigot().sendMessage(ChatApiOld.clickEvent(plugin.getYamlHandler().getLang().getString("InputIsWrong"),
				ClickEvent.Action.RUN_COMMAND, BTM.infoCommand));
		return false;
	}
	...
	public void baseCommands(final Player player, int page)
	{
		...
		int end = page*10+9;
		int last = 0;
		player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString(
				BTM.infoCommandPath+".Headline")));
		...
		pastNextPage(player, BTM.infoCommandPath, page, lastpage, BTM.infoCommand);
	}
	
	private void sendInfo(Player player, BaseConstructor bc)
	{
		player.spigot().sendMessage(ChatApiOld.apiChat(
				bc.getHelpInfo(),
