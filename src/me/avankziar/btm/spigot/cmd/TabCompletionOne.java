...
{	
	...
	public TabCompletionOne(BTM plugin)
	{
		this.plugin = plugin;
	...
	public static void renewFirstSpawn()
	{
		...
		if(new ConfigHandler(BTM.getPlugin()).enableCommands(Mechanics.FIRSTSPAWN))
		{
			ArrayList<FirstSpawn> list = ConvertHandler.convertListXII(
					BTM.getPlugin().getMysqlHandler().getTop(
							MysqlHandler.Type.FIRSTSPAWN, "`id` DESC", 0,
							BTM.getPlugin().getMysqlHandler().lastID(MysqlHandler.Type.FIRSTSPAWN)));
	...
	{
		respawn.clear();
		if(new ConfigHandler(BTM.getPlugin()).enableCommands(Mechanics.RESPAWN))
		{
			ArrayList<Respawn> list = ConvertHandler.convertListIV(BTM.getPlugin().getMysqlHandler().getTop(
					MysqlHandler.Type.RESPAWN, "`id` DESC", 0,
					BTM.getPlugin().getMysqlHandler().lastID(MysqlHandler.Type.RESPAWN)));
			for(Respawn fs : list)
			{
	...
	public static void renewDeathzone()
	{
		...
		if(new ConfigHandler(BTM.getPlugin()).enableCommands(Mechanics.DEATHZONE))
		{
			ArrayList<Deathzone> list = ConvertHandler.convertListXIII(BTM.getPlugin().getMysqlHandler().getTop(
					MysqlHandler.Type.DEATHZONE, "`id` DESC", 0,
					BTM.getPlugin().getMysqlHandler().lastID(MysqlHandler.Type.DEATHZONE)));
	...
	public List<String> onTabComplete(CommandSender sender, Command cmd, String lable, String[] args) 
	{
				...
				if (!args[0].equals("")) 
				{
					...
					} else 
					{
						if(args[0].startsWith("h:"))
						{
							if(args[0].length() == 2)
							{
								list.addAll(BTM.homes.get(player.getName()));
								Collections.sort(list);
								return list;
							} else 
							{
								String arg = args[0].split(":")[1];
								for (String homeName : BTM.homes.get(player.getName())) 
								{
						...
						} else if(args[0].startsWith("w:"))
						{
							if(args[0].length() == 2)
							{
								list.addAll(BTM.warps.get(player.getName()));
								Collections.sort(list);
								return list;
							} else
							{
								for (String warpName : BTM.warps.get(player.getName())) 
								{
				...
				|| command.equalsIgnoreCase(BTMSettings.settings.getCommands(KeyHandler.HOME_SETPRIORITY).trim())))
		{
			if (!args[0].equals("")) 
			{
				if(BTM.homes.containsKey(player.getName()))
				{
					for (String homeName : BTM.homes.get(player.getName())) 
					{
			...
			{
				if(BTM.homes.get(player.getName()) != null)
				{
					list.addAll(BTM.homes.get(player.getName()));
					Collections.sort(list);
				}
						...
						)) 
		{
			if (!args[0].equals("")) 
			{
				if(BTM.portals.containsKey(player.getName()))
				{
					for (String portal : BTM.portals.get(player.getName())) 
					{
			...
			{
				if(BTM.portals.get(player.getName()) != null)
				{
					list.addAll(BTM.portals.get(player.getName()));
				}				
			}
			Collections.sort(list);
			return list;
				...
				&& (command.equalsIgnoreCase(BTMSettings.settings.getCommands(KeyHandler.RANDOMTELEPORT).trim()))) 
		{
			if(args.length == 1)
			{
				if (!args[0].equals("")) 
				{
					if(BTM.rtp.containsKey(player.getName()))
					{
						for (String rtp : BTM.rtp.get(player.getName())) 
						{
				...
				{
					if(BTM.rtp.get(player.getName()) != null)
					{
						list.addAll(BTM.rtp.get(player.getName()));
						Collections.sort(list);
					}
					return list;
				}
				...
				&& (command.equalsIgnoreCase(BTMSettings.settings.getCommands(KeyHandler.SAVEPOINT).trim())))
		{
			if(args.length == 1)
			{
				if (!args[0].equals("")) 
				{
					if(BTM.savepoints.containsKey(player.getName()))
					{
						for (String savepointName : BTM.savepoints.get(player.getName())) 
						{
				...
				{
					if(BTM.savepoints.get(player.getName()) != null)
					{
						list.addAll(BTM.savepoints.get(player.getName()));
						Collections.sort(list);
					}
					return list;
				}
				...
				))
		{
			if (!args[0].equals("")) 
			{
				if(BTM.warps.containsKey(player.getName()))
				{
					for (String warpName : BTM.warps.get(player.getName())) 
					{
						if (warpName.startsWith(args[0])
			...
			{
				if(BTM.warps.get(player.getName()) != null)
				{
					list.addAll(BTM.warps.get(player.getName()));
					Collections.sort(list);
				}
