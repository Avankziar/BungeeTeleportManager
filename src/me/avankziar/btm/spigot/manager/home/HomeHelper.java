...
public class HomeHelper
{
	...
	public void homeCreate(Player player, String[] args)
	{
		...
		{
			player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdHome.ForbiddenServer")));
			return;
		}
		if(ForbiddenHandlerSpigot.isForbiddenToCreateWorld(plugin, Mechanics.HOME, player)
				&& !player.hasPermission(StaticValues.BYPASS_FORBIDDEN_CREATE+Mechanics.HOME.getLower()))
		{
			player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdHome.ForbiddenWorld")));
			return;
		}
		if(BTM.getWorldGuard())
		{
			...
			{
				player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdHome.WorldGuardCreateDeny")));
				return;
			}
		}
		boolean exist = false;
	...
	public void homeRemove(Player player, String[] args)
	{
		...
		player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdHome.HomeDelete")
				.replace("%name%", homeName)));
		plugin.getUtility().setHomesTabCompleter(player);
		return;
	}
	...
	public void homesDeleteServerWorld(Player player, String[] args)
	{
		...
		{
			player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdHome.HomesNotExist")
		...
		player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdHome.HomeServerWorldDelete")
				.replace("%world%", worldName)
				.replace("%server%", serverName)
	...
	public void homes(Player player, String args[])
	{
		...
		if(list.isEmpty())
		{
			player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdHome.YouHaveNoHomes")));
			return;
		}
		...
		player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdHome.HomesHeadline")
				.replace("%amount%", String.valueOf(last))));
		player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdHome.ListHelp")));
	...
	public void homeList(Player player, String args[])
	{
		...
		player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdHome.ListHeadline")
				.replace("%amount%", String.valueOf(last))));
		player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdHome.ListHelp")));
		LinkedHashMap<String, LinkedHashMap<String, ArrayList<BaseComponent>>> map = new LinkedHashMap<>();
		String sameServer = plugin.getYamlHandler().getLang().getString("CmdHome.ListSameServer");
