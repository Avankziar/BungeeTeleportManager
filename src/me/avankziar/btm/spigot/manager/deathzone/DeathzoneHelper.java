package me.avankziar.btm.spigot.manager.deathzone;
...
public class DeathzoneHelper
{
	...
	public DeathzoneHelper(BTM plugin)
	{
		this.plugin = plugin;
	}
	...
	public void simulateDeath(Player player, String[] args)
	{
		new BukkitRunnable()
		{
			...
			public void run()
			{
				...
				{
					player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdDeathzone.SimulationIsAlreadyRunning")));
					return;
				}
	...
	public void create(Player player, String[] args)
	{
		...
		{
			player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdDeathzone.DeathzoneNameAlreadyExist")));
			return;
		}
		DeathzonePosition popos = plugin.getRespawnHandler().getDeathzonePosition(player.getUniqueId());
		if(popos == null || popos.pos1 == null)
		{
			player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdDeathzone.NotPositionOneSet")));
			return;
		}
		if(popos.pos2 == null)
		{
			player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdDeathzone.NotPositionTwoSet")));
			return;
		}
		...
		player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdDeathzone.DeathzoneCreate")
				.replace("%name%", dzname)
	...
	public void remove(Player player, String[] args)
	{
		...
		if(!plugin.getMysqlHandler().exist(MysqlHandler.Type.DEATHZONE, "`displayname` = ?", dzname))
		{
			player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdDeathzone.DeathzoneNotExist")));
			return;
		}
		plugin.getMysqlHandler().deleteData(MysqlHandler.Type.DEATHZONE, "`displayname` = ?", dzname);
		player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdDeathzone.DeathzoneDelete")
				.replace("%name%", dzname)));
		return;
	}
	...
	public void list(Player player, String[] args)
	{
		...
		if(list.isEmpty())
		{
			player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdDeathzone.ThereIsNoDeathzone")));
			return;
		}
		...
		for(Deathzone r : list)
		{
			...
			s += " &9| ";
			map = rh.mapping(r, map, ChatApiOld.hoverEvent(
					s,
		...
		player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdDeathzone.ListHeadline")
				.replace("%amount%", String.valueOf(last))));
		player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdDeathzone.ListHelp")));
		player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdDeathzone.ListHelpII")
	...
	public void info(Player player, String[] args)
	{
		...
		player.spigot().sendMessage(
				ChatApiOld.generateTextComponent(plugin.getYamlHandler().getLang().getString("CmdDeathzone.InfoHeadlineI")
		...
		player.spigot().sendMessage(ChatApiOld.generateTextComponent(plugin.getYamlHandler().getLang().getString("CmdDeathzone.InfoLocationI")
				.replace("%value%", Utility.getLocationV2(dz.getPosition1()))
				.replace("%dz%", dz.getDisplayname())));
		player.spigot().sendMessage(ChatApiOld.generateTextComponent(plugin.getYamlHandler().getLang().getString("CmdDeathzone.InfoLocationII")
				.replace("%value%", Utility.getLocationV2(dz.getPosition2()))
				.replace("%dz%", dz.getDisplayname())));
		player.spigot().sendMessage(ChatApiOld.generateTextComponent(plugin.getYamlHandler().getLang().getString("CmdDeathzone.InfoCategory")
				.replace("%cmd%", BTMSettings.settings.getCommands(KeyHandler.DEATHZONE_SETCATEGORY).trim())
				.replace("%value%", dz.getCategory())
				.replace("%dz%", dz.getDisplayname())));
		player.spigot().sendMessage(ChatApiOld.generateTextComponent(plugin.getYamlHandler().getLang().getString("CmdDeathzone.InfoSubCategory")
				.replace("%cmd%", BTMSettings.settings.getCommands(KeyHandler.DEATHZONE_SETCATEGORY).trim())
				.replace("%value%", dz.getSubCategory())
				.replace("%dz%", dz.getDisplayname())));
		player.spigot().sendMessage(ChatApiOld.generateTextComponent(plugin.getYamlHandler().getLang().getString("CmdDeathzone.InfoPriority")
				...
				.replace("%dz%", dz.getDisplayname())));
		player.spigot().sendMessage(ChatApiOld.generateTextComponent(plugin.getYamlHandler().getLang().getString("CmdDeathzone.InfoDeathzonepath")
	...
	public void setCategory(Player player, String[] args)
	{
		...
		player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdDeathzone.SetCategory")
				.replace("%dz%", dzname)
				.replace("%cat%", cat)
				.replace("%subcat%", subcat)));
		return;
	...
	public void setName(Player player, String[] args)
	{
		...
		plugin.getMysqlHandler().updateData(MysqlHandler.Type.DEATHZONE, dz, "`displayname` = ?", dzname);
		player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdDeathzone.SetName")
	...
	public void setPriority(Player player, String[] args)
	{
		...
		dz.setPriority(prio);
		plugin.getMysqlHandler().updateData(MysqlHandler.Type.DEATHZONE, dz, "`displayname` = ?", dzname);
		player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdDeathzone.SetPriority")
				.replace("%dz%", args[0])
				.replace("%prio%", args[1])));
		return;
	}
	...
	public void setDeathzonePath(Player player, String[] args)
	{
		...
		plugin.getMysqlHandler().updateData(MysqlHandler.Type.DEATHZONE, dz, "`displayname` = ?", dzname);
		player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdDeathzone.SetDeathzonepath")
				.replace("%dz%", dzname)
	...
	public void mode(Player player, String[] args)
	{
		...
		if(RespawnHandler.deathzoneCreateMode.contains(player.getUniqueId()))
		{
			...
			player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdDeathzone.DeathzoneCreationMode.Removed")));
		} else
		{
			RespawnHandler.deathzoneCreateMode.add(player.getUniqueId());
			player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdDeathzone.DeathzoneCreationMode.Added")));
		}
	}
}