...
public class WarpHandler
{
	...
	public int compareGlobalWarps(Player player, boolean message)
	{
		...
		if(i >= 0 || globalLimit == 0)
		{
			if(message)
			{
				player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdWarp.TooManyWarpsGlobal")
	...
	public int compareServerWarps(Player player, boolean message)
	{
		...
		if(clusterBeforeServer && serverIsInCluster)
		{
			...
			if(i >= 0 || serverLimit == 0)
			{
				if(message)
				{
					player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdWarp.TooManyWarpsServerCluster")
			...
			if(i >= 0 || serverLimit == 0)
			{
				if(message)
				{
					player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdWarp.TooManyWarpsServer")
							.replace("%amount%", String.valueOf(serverLimit))));
				}
			}
			return i;
	...
	public int compareWorldWarps(Player player, boolean message)
	{
		...
		if(clusterActive && worldIsInCluster)
		{
			...
			if(i >= 0 || worldLimit == 0)
			{
				if(message)
				{
					player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdWarp.TooManyWarpsWorld")
			...
			BaseComponent bct)
	{
		if(map.containsKey(warp.getLocation().getServer()))
		{
			...
			} else
			{
				...
				bc.add(ChatApiOld.tctl("  &e"+warp.getLocation().getWorldName()+": "));
				bc.add(bct);
