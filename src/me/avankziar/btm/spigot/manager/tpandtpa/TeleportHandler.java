...
public class TeleportHandler
{
	...
	{
		player.spigot().sendMessage(
				ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdTp.SendRequest")
	...
	public void tpAccept(Player player, Teleport teleport)
	{
		...
		player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdTp.RequestInProgress")));
	...
	public void tpToggle(Player player)
	{
		...
		if(back.isToggle())
		{
			...
			player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdTp.ToggleOff")));
		} else
		{
			back.setToggle(true);
			plugin.getBackHandler().sendBackObject(player, back, true);
			player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdTp.ToggleOn")));
		}
	}
		...
		if(Bukkit.getPlayer(UUID.fromString(playeruuid)) != null)
		{
			...
			new BukkitRunnable()
			{
				...
				public void run()
				{
					...
					player.teleport(targets);
					player.spigot().sendMessage(ChatApiOld.tctl(plugin.getYamlHandler().getLang().getString("CmdTp.SilentPlayerTeleport")
							.replace("%playerto%", targets.getName())));
				}
