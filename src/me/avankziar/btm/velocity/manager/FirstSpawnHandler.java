	...
	public void pluginMessage(PluginMessageEvent event) throws IOException
	{
		DataInputStream in = new DataInputStream(new ByteArrayInputStream(event.getData()));
	       ...
	       if(task.equals(StaticValues.FIRSTSPAWN_PLAYERTOPOSITION))
	       {
			   ...
			   teleportPlayer(playerName, location, delay);
			   return;
	       } else if(task.equals(StaticValues.FIRSTSPAWN_DOCOMMANDS))
	       {
	    	   String uuid = in.readUTF();
	    	   Optional<Player> opplayer = plugin.getServer().getPlayer(UUID.fromString(uuid));
	    	   if(opplayer.isEmpty())
	    	   {
	    		   return;
	    	   }
	    	   ...
	    	   for(int i = 0; i < size; i++)
	    	   {
	    		   ...
	    		   plugin.getServer().getCommandManager().executeAsync(player, cmd.replace("%player%", player.getUsername()));
	    	   }
	    	   ...
	    	   for(int i = 0; i < sizeII; i++)
	    	   {
	    		   ...
	    		   plugin.getServer().getCommandManager().executeAsync(plugin.getServer().getConsoleCommandSource(),
	    				   cmd.replace("%player%", player.getUsername()));
	    	   }
	       }
	       return;
	}
	
	public void teleportPlayer(String playerName, ServerLocation location, int delay)
	{
		plugin.getServer().getScheduler().buildTask(plugin, (task) ->
		{
			...
			sendPluginMessage(player, server.get(), location, r);
		}).delay(delay, TimeUnit.MILLISECONDS).schedule();
	}
	
	private void sendPluginMessage(Player player, RegisteredServer server, ServerLocation location, CompletableFuture<Result> result)
	{
		plugin.getServer().getScheduler().buildTask(plugin, (task) ->
		{
			if(result != null)
	        ...
	        server.sendPluginMessage(BTM.getMCID(StaticValues.FIRSTSPAWN_TOSPIGOT), streamout.toByteArray());
