...
public class CustomMessageListener implements Listener
{
	...
	public void onTeleportMessage(PluginMessageEvent event) throws IOException
	{
        ...
        if (!event.getTag().equalsIgnoreCase(StaticValues.CUSTOM_TOBUNGEE)) 
        {
            return;
        }
	...
	public void preTeleportPlayerToPlayerForceUse(Teleport teleport, String errormessage, int delay)
	{
		...
		if(to == null)
		{
			from.sendMessage(ChatApiOld.tctl(errormessage));
			return;
		}
	...
	public void teleportPlayer(ProxiedPlayer sender, ProxiedPlayer target, int delay)
	{
		...
		{
			sender.sendMessage(ChatApiOld.tctl("Server is unknow!"));
			return;
		}
		...
		plugin.getProxy().getScheduler().schedule(plugin, new Runnable()
		{
			...
			public void run()
			{
			    ...
			    target.getServer().sendData(StaticValues.CUSTOM_TOSPIGOT, streamout.toByteArray());
			    return;
			...
			String message, int delay)
	{
		...
		{
			player.sendMessage(ChatApiOld.tctl(errorServerNotFound));
			return;
		}
	...
	public void teleportPlayerToPositionPost(final ProxiedPlayer player, final ServerLocation location, String message, int delay)
	{
		...
		plugin.getProxy().getScheduler().schedule(plugin, new Runnable()
		{
			...
			public void run()
			{
		        ...
		        plugin.getProxy().getServerInfo(location.getServer()).sendData(StaticValues.CUSTOM_TOSPIGOT, streamout.toByteArray());
			    return;
			}
		}, delay, TimeUnit.MILLISECONDS);
	}
