	...
	public void onTeleportMessage(PluginMessageEvent event) throws IOException
	{
        ...
        if(task.equals(StaticValues.TP_EXISTPENDING))
        {
        	...
        	if(plugin.getProxy().getPlayer(toName) == null)
        	{
        		...
        		{
        			plugin.getProxy().getPlayer(fromName).sendMessage(ChatApiOld.tctl(error));
        		}
        		return;
        	}
        ...
        } else if(task.equals(StaticValues.TP_SENDMESSAGE))
        {
        	...
        	if(plugin.getProxy().getPlayer(toName) == null)
        	{
        		if(returns)
        		{
        			plugin.getProxy().getPlayer(fromName).sendMessage(ChatApiOld.tctl(returnmessage));
        		}
        		return;
        	}
        	plugin.getProxy().getPlayer(toName).sendMessage(ChatApiOld.tctl(message));
        	return;
        } else if(task.equals(StaticValues.TP_SENDTEXTCOMPONENT))
        {
        	...
        	plugin.getProxy().getPlayer(toName).sendMessage(ChatApiOld.generateTextComponent(message));
        	return;
    			...
    			public void run()
    			{
    				if(TeleportHandler.getPendingTeleports().containsKey(fromName))
    	        	{
        	        	...
        	        	if(sendMessage)
        	        	{
        	        		if(plugin.getProxy().getPlayer(fromName) != null)
        	        		{
        	        			plugin.getProxy().getPlayer(fromName).sendMessage(ChatApiOld.tctl(messageFrom));
        	        		}
        	        		if(plugin.getProxy().getPlayer(toName) != null)
        	        		{
        	        			plugin.getProxy().getPlayer(toName).sendMessage(ChatApiOld.tctl(messageTo));
        	        		}
        ...
        } else if(task.equals(StaticValues.TP_DENY))
        {
        	...
        	if(!TeleportHandler.getPendingTeleports().containsKey(fromName))
        	{
        		if(sendMessage)
        		{
        			plugin.getProxy().getPlayer(toName).sendMessage(ChatApiOld.tctl(errormessage));
        			return;
        		}
        	}
        	TeleportHandler.getPendingTeleports().remove(fromName);
    		plugin.getProxy().getPlayer(fromName).sendMessage(ChatApiOld.tctl(message));
    		plugin.getProxy().getPlayer(toName).sendMessage(ChatApiOld.tctl(message));
        	return;
        } else if(task.equals(StaticValues.TP_CANCEL))
        {
        	...
        	if(!TeleportHandler.getPendingTeleports().containsKey(fromName))
        	{
        		if(sendMessage)
        		{
        			plugin.getProxy().getPlayer(fromName).sendMessage(ChatApiOld.tctl(errormessage));
        			return;
        		}
        	}
