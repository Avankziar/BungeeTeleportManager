		...
		if (conn != null) {
			try 
			{
				String sql = "INSERT INTO `" + plugin.getMysqlHandler().tableNameIII 
						+ "`(`player_uuid`, `player_name`, `back_location`, `tp_toggle`, `show_forbiddenarea`) " 
						+ "VALUES(?, ?, ?, ?, ?)";
				preparedStatement = conn.prepareStatement(sql);
		        ...
		        preparedStatement.setBoolean(5, b.isShowforbiddenarea());
		...
		if (conn != null) 
		{
			try 
			{
				String data = "UPDATE `" + plugin.getMysqlHandler().tableNameIII
						+ "` SET `player_uuid` = ?,"
						+ " `player_name` = ?, `back_location` = ?, `tp_toggle` = ?, `show_forbiddenarea` = ?" 
						+ " WHERE "+whereColumn;
			...
			{			
		        ...
		        while (result.next()) 
		        {
		        	return new Back(UUID.fromString(result.getString("player_uuid")),
		        			...
		        			result.getBoolean("tp_toggle"), result.getBoolean("show_forbiddenarea"));
		        }
		    } catch (SQLException e) 
			{
				  BungeeTeleportManager.log.warning("Error: " + e.getMessage());
