...
public interface Table02
{
	...
	default boolean existII(BungeeTeleportManager plugin, String whereColumn, Object... object) 
	{
		...
		if (conn != null) 
		{
			try 
			{			
				String sql = "SELECT `id` FROM `" + MysqlHandler.Type.PORTAL.getValue() 
	...
	default boolean deleteDataII(BungeeTeleportManager plugin, String whereColumn, Object... whereObject)
	{
		...
		try 
		{
			String sql = "DELETE FROM `" + MysqlHandler.Type.PORTAL.getValue() + "` WHERE "+whereColumn;
			preparedStatement = conn.prepareStatement(sql);
			int i = 1;
	        for(Object o : whereObject)
	        {
	...
	default int lastIDII(BungeeTeleportManager plugin)
	{
		...
		if (conn != null) 
		{
			try 
			{			
				String sql = "SELECT `id` FROM `" + MysqlHandler.Type.PORTAL.getValue() + "` ORDER BY `id` DESC LIMIT 1";
		        preparedStatement = conn.prepareStatement(sql);
